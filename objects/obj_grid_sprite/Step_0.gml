...
if (update) {
	...
	var unitSpriteWidth = sprite_get_width(object_get_sprite(unit.object_index));
	var unitSpriteHeight = sprite_get_height(object_get_sprite(unit.object_index));
	
	//show_message("Sprite Width: " + string(unitSpriteWidth))
	//show_message("Sprite Height: " + string(unitSpriteHeight))
	var unitSlackX = cell_size_x - unitSpriteWidth;
	var unitSlackY = cell_size_y - unitSpriteHeight;
	
	//show_message("Unit Slack X:" + string(unitSlackX))
	//show_message("Unit Slack Y: " + string(unitSlackY))

	unit.image_xscale = (unitSpriteWidth + unitSlackX) / unitSpriteWidth
	unit.image_yscale = (unitSpriteHeight + unitSlackY) / unitSpriteHeight
	unit.x = unit.pos_x * cell_size_x + (unit.pos_x * grout)
	unit.y = unit.pos_y * cell_size_y + (unit.pos_y * grout)
}